...
lock_record = df_locks[df_locks["Id"] == 51064].copy()

if lock_record.empty:
    logger.error("Lock not found!")
    exit(1)

logger.info("Running group_complexes for lock %s...", lock_record.iloc[0]["Name"])
...
for b in lock_node["berths"]:
    if b["id"] == 27256967:
        logger.info(
            "✅ Found Berth 27256967 (Relation: %s, Dist: %sm)",
            b["relation"],
            b["dist_m"],
        )
        exit(0)
