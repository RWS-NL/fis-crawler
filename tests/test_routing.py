...
class TestRoutingEngine(unittest.TestCase):
    ...
    def test_check_edge_constraints_soft(self):
        ...
        # 3. Air draft check
        # Fixed bridge, too low
        struct_low = {"chambers": [], "openings": [{"type": "VST", "height_closed": 4.0}]}
        pen, viols = check_edge_constraints_soft(d_edge, struct_low, ship_dims)
        self.assertEqual(pen, 1000.0)
        self.assertEqual(viols[0]["type"], "height")
        
        # Movable, fits
        struct_mov = {"chambers": [], "openings": [{"type": "OPH", "height_closed": 3.0}]}
        pen, viols = check_edge_constraints_soft(d_edge, struct_mov, ship_dims)
        self.assertEqual(pen, 0.0)
        
        # 4. Depth constraint with safety margin
        d_depth_ok = {"mindepth_lower": -2.5}
        pen, viols = check_edge_constraints_soft(d_depth_ok, {"chambers": [], "openings": []}, ship_dims)
        self.assertEqual(pen, 0.0)
        
        d_depth_shallow = {"mindepth_lower": -2.1}
        pen, viols = check_edge_constraints_soft(d_depth_shallow, {"chambers": [], "openings": []}, ship_dims)
        self.assertEqual(pen, 1000.0)
        self.assertEqual(viols[0]["type"], "depth")
