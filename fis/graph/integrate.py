...
def find_geometric_border_connections(
    ...
    fis_gdf = gpd.GeoDataFrame(
        {'node_id': fis_ids, 'geometry': fis_points}, 
        crs='EPSG:4326'
    ).to_crs('EPSG:32631')
    ...
    logger.info("Found %d EURIS cross-border edges with %d unique NL bridgeheads", 
                len(border_edges), len(bridgeheads))
    ...
    for bh in bridgeheads:
        ...
        if p:
            ...
            if min_dist < distance_threshold:
                ...
                logger.debug("Matched %s -> FIS:%s (%.1fm)", 
                           bh, matches[bh]['fis_node'], min_dist)
