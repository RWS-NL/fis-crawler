    ...
    if hasattr(fw_obj, "geometry") and fw_obj.geometry:
         ...
         if hasattr(lock, "geometry") and lock.geometry:
             try:
                 ...
                 projected_point = fairway_geom.interpolate(projected_dist)
                 ...
                 # Determine Km range
                 start_km = float(fw_obj["RouteKmBegin"])
                 end_km = float(fw_obj["RouteKmEnd"])
                 direction = fw_obj["Direction"] # N/A or similar?
                 ...
                 logger.warning(f"Error processing fairway geometry for lock {lock['Id']}: {e}")
                 
    return fairway_data

def find_nearby_berths(lock, berths_gdf, geometry_before_wkt=None, geometry_after_wkt=None):
    """
...
def find_chamber_doors(chamber_geom, split_point, merge_point):
    ...
    target_geom = chamber_geom.boundary if chamber_geom.geom_type in ['Polygon', 'MultiPolygon'] else chamber_geom
